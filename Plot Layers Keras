...
##################
## PRINT INTERMEDIATE LAYERS - ENTER OUTPUT LAYER AT "part"
for i in range(0,14):
    print(i,denoise.layers[i].name)

denoise.summary()
part=4
denoise.layers[part].name

get_0_layer_output = K.function([denoise.layers[0].input, K.learning_phase()],[denoise.layers[part].output])

np.shape(get_0_layer_output([x_train, 0])[0])

plt.figure(figsize=(10,10))
for i in range(0,np.shape(get_0_layer_output([x_train, 0])[0])[3]):    
    ax=plt.subplot(4, 5, i+1)
    plt.imshow(get_0_layer_output([x_train, 0])[0][0][i])
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.show()
