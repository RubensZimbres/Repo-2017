...
with writer.saving(fig, "Twitter_REAL_Time_Topic_2.mp4", 100):
    while t<20:
        data=[]
        ...
        h=[]
        label=[]
        ax1.set_title('NATURAL LANGUAGE PROCESSING\n\n'+'TOPIC MODELING at TWITTER HASHTAG: '+'trump',fontweight="bold")
        for i in range(0,4):
            label.append('Topic {}'.format([0,1,2,3][i]))
            ax1.scatter(X_tsne[:, 0], X_tsne[:, 1],c=cc,cmap=cm,marker='o',s=100)
            h1,=ax1.plot(1,1,color=colors[i],linewidth=3)
            h.append(h1)
        ax1.legend(h,label,loc="upper left")
