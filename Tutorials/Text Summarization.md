...
plt.title('TEXT RATIONALITY - Structure of Arguments (0,1)\n'+'WIKIPEDIA webpage Statistical Inference\n'+'Gensim Only 2 Topics',fontweight="bold")
plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c='',marker='.', s=2,label='Topic {0})'.format(cc))
...
a5=[]
for i in range(0,len(tokens_txt)):
    a5.append(np.sum([Counter(tokens_txt[i])[x] for x in tokens_lsi[3]]))
topic5=norm(a5)
...
[print(i,documents[i],'|| Match={}'.format(topic5[i]),'\n') for i in np.where(topic5>threshold)[0]]
